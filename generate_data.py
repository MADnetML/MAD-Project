def save_data(number_of_samples, measurement_size, kernel_size, SNR=2, training=False, validation=False):
    ...
    for i in range(number_of_samples):
        ...
        temp_measurement, temp_kernel, temp_activation_map = Y_factory(E + E_shift, (n1, n2),
                                                                       ...
                                                                       SNR)
        if training:
            np.save(os.getcwd() + '/training_dataset/kernel_%d.pt' % i, fix_levels(temp_kernel, E))
            np.save(os.getcwd() + '/training_dataset/measurement_%d.pt' % i, fix_levels(temp_measurement, E))
            np.save(os.getcwd() + '/training_dataset/activation_%d.pt' % i, fix_levels(temp_activation_map, E))
        elif validation:
            np.save(os.getcwd() + '/validation_dataset/kernel_%d.pt' % i, fix_levels(temp_kernel, E))
            np.save(os.getcwd() + '/validation_dataset/measurement_%d.pt' % i, fix_levels(temp_measurement, E))
            np.save(os.getcwd() + '/validation_dataset/activation_%d.pt' % i, fix_levels(temp_measurement, E))
